...
/* let numbers = [23, 5, 67, 12, 88, 34];
// 반복문 없이 풀이
let max = numbers[0]; // 배열명[인덱스] : 특정한 인젝스 번째 요소 값 호출 //첫번째 값을 최댓값 지정
...
*       line = 5                star = 1 
*/
// line은 1부터 5까지 1씩 증가 하면서 '\n' 출력
// star은 1부터 (마지막줄수 - 현재 줄수 + 1) 1씩 증가하면서 '*' 출력        // 줄 마다 별 출력 ? 별 마다 줄 출력 ?
/*
...
for( let index = 0; index <= userNames.length - 1; index++ ){
    if( userNames[index].indexOf('솔') != -1 ){     // 문자열은 배열이다.
        console.group( userNames[ index ] )
        break   // for 탈출/종료    // .indexOf( 찾을 값 ) : 찾을값이 존재하면 인덱스 반환하고 없으면 -1 반환
    }
}   // for end
*/
...
/*
let seatLayout = [['A1', 'A2', 'A3'], ['B1', 'B2', 'B3'], ['C1', 'C2', 'C3']];
// 1. 행 먼저 반복문 이용하여 조회한다. 1행부터 3행까지 1씩 증가하면 행 출력
...
for( let index = 0; index <= numbers.length - 1; index++ ){
    ...
    if( newNumbers.indexOf( num ) == -1 ){  // 새로운 배열에 존재하지 않으면
        newNumbers.push( num ) // 새로운 배열에 저장한다 .push( 새로운 값 )
    }
}
...
let numbers = [5, 3, 4, 1, 2];
// 비교기준(인덱스) :   0        1        2        3
// 바교대상 :       1 2 3 4   2 3 4      3 4       4
for( let i = 0; i <= numbers.length - 1; i++){ // i는 0부터 마지막인덱스 전까지
    for( let j = i + 1; j <= numbers.length - 1; j++){  // j는 i + 1 부터 마지막 인덱스까지
        if( numbers[i] > numbers[j] ){
            let temp = numbers[i]   // 스왑(두 변수간 값 교체)
...
for( let index = 0; index <= products.length - 1; index++ ){
    if( products[index] == 상품명 ){ // 만약에 index 번째 값과 입력받은 값과 같으면
        // 2. 재고 비교
        if( stock[index] >= 수량 ){ // 만약에 index 번째 재고값이 이 입력받은 수량 이상이면
            console.log('구매 완료')
            stock[index] = stock[index] - 수량 // 재고 차감
        }else{
...
for( let index = 0; index <= movieNames.length - 1; index++){   // index번째 영화 이름을 대입
    html += movieNames[index]; // index번째 영화 이름을 대입
    // 평점(별) 출력하기
    for( let star = 1; star <= 10; star++ ){ // 만약에 현재 별이 평점보다 이하이면 검은별
...
for( let i = 0; i <= carNumbers.length-1; i++){
    ...
    let car = String(carNumbers[i])
    if(fee>20000) fee = '20000원'
    if(fee<1000) fee = '1000원'
    html += `${car} : 최종 요금: ${fee} <br />`
}
