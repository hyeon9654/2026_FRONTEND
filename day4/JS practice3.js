...
let A = (prompt('필요한 과일?'));       // 1. 입력 2. 저장
let 존재여부 = fruitList.includes(A);   // 3. 처리 : 입력받은 값이 fruitList 존재여부
if ( 존재여부 ){ // 출력 (분기)
...
console.log('최종 결제 금액은 ' + 결제금액 + '원입니다.');
...
let 월 = Number(prompt('1부터 12 사이의 월을 입력하세요.'));
...
/*
let 숫자1 = parseInt(prompt('1번째 숫자를 입력하세요.'));
let 숫자2 = parseInt(prompt('2번째  숫자를 입력하세요.'));
let 숫자3 = parseInt(prompt('3번째 숫자를 입력하세요.'));
...
if( (연도%4 == 0 && 연도%100 != 0) || 연도%400 == 0 ){
    console.log( 연도 + '는 윤년입니다');
}else{
    console.log( 연도 + '는 평년입니다' );
}
*/
/* 
문제 7: 세 수 오름차순 정렬
...
// 3. 처리 : 17, 4, 8
// 1. 정수1 -> 정수2 비교 후 만약에 더 정수1이 더 크면 자리 바꾸기( 스왑 )
...
// 2. 정수1 -> 정수3 비교 후 만약에 더 정수1이 더 크면 자리 바꾸기
if( 정수1 > 정수3){ 
    let temp = 정수1;
    정수1 = 정수3;
    정수3 = temp;
}
// 3. 정수2 -> 정수3 비교 후 만약에 더 정수2이 더 크면 자리 바꾸기
if( 정수2 > 정수3){
    let temp = 정수2;
    정수2 = 정수3
    정수3 = temp;
...
// 처리 :
// 플레이어1 이기는 수 : if( player1 == 0 && player2 == 2 || player1 == 1 && player2 == 0 ||
// player1 == 2 && player2 == 1)
if ( player1 == 0 ) {//1. 플레이어1 가위 일때
    if( player2 == 0 ) console.log("무승부");
...
}else if (player1 == 2){//3. 플레이어1 보 일때
...
//사용자로부터 차량 번호를 입력받아, carArray에서 해당 차량을 찾은 뒤 locationArray에 있는 그 차량의 
//주차 위치를 찾아 출력하는 프로그램을 작성하시오. 만약 해당하는 차량 번호가 없다면 '차량이 존재하지 않습니다.'를 출력하시오.
let carArray = [ '250어7142', '142가7415', '888호8888' ];
let locationArray = [ 'A1', 'B3', 'C2' ];
let 차량번호 = prompt( "차량번호를 입력하시오.");
let 존재여부 = carArray.indexOf(차량번호);
let 주차위치 = locationArray[존재여부]
if(존재여부>=0){
    console.log(주차위치);
}else{
...
let courseList = [ '수학', '영어', '과학', '국어'];
let 제외 = String(prompt('제외하고 싶은 과목을 쓰세요.'));
let 체크 = courseList.indexOf(제외)
if(체크 < 0){
    console.log(`해당 과목은 신청 목록에 없습니다.`)
}else{
    courseList.splice(체크, 1)
